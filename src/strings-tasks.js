...
function removeLastOccurrences(str, value) {
  ...
  return str.substring(0, ind) + str.substring(ind + value.length, str.length);
}
...
function isPalindrome(str) {
  let j = 0;
  const strWithoutSpace = str.toLocaleLowerCase().replaceAll(/[,!? ]/g, '');
  for (let i = strWithoutSpace.length - 1; i >= 0; i -= 1) {
...
function findLongestWord(sentence) {
  ...
  const arrWithLength = arr
    .map((el) => [el, el.length])
    .sort((a, b) => b[1] - a[1]);
  return arrWithLength[0][0];
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      ...
      resStr += String.fromCharCode(str.charCodeAt(i) + 13);
    } else if (
      (str.charCodeAt(i) >= 110 && str.charCodeAt(i) <= 122) ||
      (str.charCodeAt(i) >= 77 && str.charCodeAt(i) <= 90)
    ) {
      resStr += String.fromCharCode(str.charCodeAt(i) - 13);
    } else {
      resStr += str[i];
